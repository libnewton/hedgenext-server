...
// auth method
config.isFacebookEnable = false
config.isGoogleEnable = false
config.isDropboxEnable = false
config.isTwitterEnable = false
config.isEmailEnable = config.email
config.isOpenIDEnable = config.openID
config.isGitHubEnable = false
config.isGitLabEnable = false
config.isMattermostEnable = false
config.isLDAPEnable = false
config.isSAMLEnable = false
...
if (['filesystem', 's3', 'minio', 'imgur', 'nextcloud', 'lutim'].indexOf(config.imageUploadType) === -1) {
  logger.error('"imageuploadtype" is not correctly set. Please use "filesystem", "s3", "minio", "nextcloud", "lutim" or "imgur". Defaulting to "nextcloud"')
  config.imageUploadType = 'nextcloud'
}
