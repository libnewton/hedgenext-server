...
async function save_to_nextcloud(markdown_data, note_id,fid,docId,user_id,host) {
  const app_base = host.match(/(.+\/apps\/)/)[1] + 'hedgenext'
  const fpath = host.match(/.+filePath=(.+)(&?)/)[1]
  const resp = await fetch(`${app_base}/callback/${fpath}/put?user_id=${user_id}&fid=${fid}`, {})

}
async function handle_handoff (note_id, handoff_data) {
  const {fid,docId,user_id, host} = JSON.parse(Buffer.from(handoff_data, 'base64').toString('utf8'))
...
exports.newNote = async function (req, res, body) {
  ...
  models.Note.create({
    ...
    const noteDest = (note.alias ? note.alias : models.Note.encodeNoteId(note.id))
    if (handoff_data) {
      await handle_handoff(noteDest, handoff_data)
    }
    return res.redirect(config.serverURL + '/' + noteDest)
  }).catch(function (err) {
