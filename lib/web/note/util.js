...
exports.findNote = function (req, res, isPublic, callback, include, createIfNotFound = true) {
...
exports.save_to_nextcloud = async function (handoff_data, note_id, body) {
  try {
    ...
    await fetch(config.ncHost + '/apps/hedgenext/hedge/post', {
      method: 'POST',
      body: form,
...
exports.newNote = async function (req, res, body) {
  ...
  if (req.isAuthenticated()) {
    owner = req.user.id
  } else if (!config.allowAnonymous && (!config.freeToken || config.freeToken !== req.query.token)) {
