...
statusRouter.post('/pdfexport', bodyParser.urlencoded({ extended: true }), async function (req, res) {
  try {
    ...
    formd.append('url', req.body.url)
    formd.append('paperWidth', '8.27')
    formd.append('paperHeight', '11.7')
    formd.append('marginTop', '0.25')
    formd.append('marginBottom', '0.25')
    formd.append('marginLeft', '0.25')
    formd.append('marginRight', '0.25')
    formd.append('waitForExpression', 'window.HAS_LOADED = Object.keys(window.MathJax?.Ajax?.loading ?? [1]).length === 0; if(window.HAS_LOADED && !window.LOADED_AT) {window.LOADED_AT = Date.now()}; (window.HAS_LOADED && Date.now() - window.LOADED_AT > 750)')
    const resp = await fetch(`${config.gotenbergPDF}/forms/chromium/convert/url`, {
      method: 'POST',
      body: formd,
