...
module.exports = function (sequelize, DataTypes) {
  ...
  User.getProfile = function (user, withTemplates = false) {
    if (!user) {
      return null
    }
    const baseUser = user.profile ? User.parseProfile(user.profile) : (user.email ? User.parseProfileByEmail(user.email) : null)
    if (baseUser && withTemplates) {
      baseUser.templates = user.templates
    }
    return baseUser
  }
