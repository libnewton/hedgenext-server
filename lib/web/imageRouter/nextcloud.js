...
exports.uploadImage = function (imagePath, callback) {
  ...
  if (!config.ncUser || !config.ncPassword || !config.ncFolder || !config.ncHost) {
    callback(new Error('Nextcloud credentials are missing'), null)
    return
  }
  const fname = randomBytes(10).toString('hex') + '.' + imagePath.split('.').pop()
  const url = config.ncHost + '/remote.php/dav/files/' + config.ncUser + '/' + config.ncFolder + '/' + fname
  ...
  fetch(url, options).then(function (res) {
    if (res.status < 400) {
      fetch(config.ncHost + '/ocs/v2.php/apps/files_sharing/api/v1/shares?format=json', {
        method: 'POST',
        headers: {
          Authorization: 'Basic ' + Buffer.from(config.ncUser + ':' + config.ncPassword).toString('base64'),
